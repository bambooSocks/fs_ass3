...
// tests ex3
// result : []
let test5 = extractFreeIdent Set.empty (Let ("x", Sum(Const 2,Ident "x"), Sum(Ident "x",Const 3)))
// result: [Ident "y"]
let test6 = extractFreeIdent Set.empty (Let ("x", Sum(Const 2,Ident "x"), Sum(Ident "y",Const 3)))
// result  [Ident "y"]
let test7 = extractFreeIdent Set.empty (Let ("x", Sum(Const 2,Ident "x"), Sum(Ident "y",Const 3)))
// result : []
let test8 = extractFreeIdent Set.empty (Let("x", Const 2, Sum(Ident "x", Ident "x")))
...
// extractFreeIdent2: Set<ExprTree2> -> ExprTree2 -> Set<ExprTree2>
...
let test15 = extractFreeIdent2 Set.empty (App("h", [Const 2; Ident "x"; Const 3;Let ("a", Const 3 ,Sum (Ident "b", Const 3) )]))
...
// set.union does not preserve the initial order of the identifiers occuring in the tree
let test16 = extractFreeIdent2 Set.empty (App("h", [Const 2; Ident "x"; Const 3;Sum(Sum(Ident "x",Ident "y"),Sum(Ident "z",Ident "z"));Let ("a", Const 3 ,Sum (Ident "b", Const 3) )]))

// tests subst2 
// checks if the last branch of the function is executed and returns
// App("h", [Const 2; Const 10; Const 3;Let ("a", Const 3 ,Sum (Const 10, Const 3) )])
